//uart0 code taken from common terminal interface embedded I project and modified for RTOS nano use.
//Author: IDK probably mostly Losh, with gaps that Ryan Smith filled in.
...
char getcUart0()
{
    while (UART0_FR_R & UART_FR_RXFE)               // wait if uart0 tx fifo empty
    {
        yield();                                    //yield if waiting.
    }              
